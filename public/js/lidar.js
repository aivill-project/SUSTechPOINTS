...
function Lidar(sceneMeta, world, frameInfo) {
    ...
    loader.load(
      ...
      function (pcd) {
        ...
        console.log(
          ...
          'parse pionts ',
          _self.points_parse_time - _self.world.create_time,
          'ms'
        )
        ...
        console.log(
          ...
          _self.frameInfo.frame,
          'loaded pionts ',
          _self.points_load_time - _self.world.create_time,
          'ms'
        )
      ...
      function () {
        ...
        console.log(
          ...
          _self.frameInfo.frame,
          'read file ',
          _self.points_readfile_time - _self.world.create_time,
          'ms'
        )
      }
    )
  ...
  ;(this.points_index_grid_size = 1),
    (this.get_position_key = function (x, y, z) {
      return [Math.floor(x / this.points_index_grid_size), Math.floor(y / this.points_index_grid_size), Math.floor(z / this.points_index_grid_size)]
    })
  ...
  this.grow_box = function (box, min_distance, init_scale_ratio) {
    ...
    let inside_points = 0
    translated_cand_points.forEach((tp, i) => {
      if (Math.abs(tp[0]) > box.scale.x / 2 + 0.01 || Math.abs(tp[1]) > box.scale.y / 2 + 0.01 || Math.abs(tp[2]) > box.scale.z / 2 + 0.01) {
        return
      } else {
    ...
    translated_cand_points.forEach(tp => {
      if (tp[0] > extreme.max.x || tp[0] < extreme.min.x || tp[1] > extreme.max.y || tp[1] < extreme.min.y || tp[2] > extreme.max.z || tp[2] < extreme.min.z) {
      } else {
