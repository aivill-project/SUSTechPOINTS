...
function Editor(editorUi, wrapperUi, editorCfg, data, name = 'editor') {
  ...
  this.addRangeCircle = function () {
    ...
    var box = new THREE.LineSegments(bbox, new THREE.LineBasicMaterial({ color: 0x888800, linewidth: 2, opacity: 2.0, transparent: true }))
  ...
  this.handleFastToolEvent = function (event) {
    ...
    switch (event.currentTarget.id) {
      ...
      case 'label-batchedit':
        {
          if (!this.checkBoxTrackId()) break

          if (!this.checkAnnBeforeBatchEdit()) break
  ...
  this.handleContextMenuEvent = function (event) {
    switch (event.currentTarget.id) {
      ...
      case 'cm-show-frame-info':
        {
          let info = { 'scend-id': this.data.world.frameInfo.scene, 'frame': this.data.world.frameInfo.frame }
  ...
  this.resetView = function (targetPos) {
    if (!targetPos) targetPos = { x: 0, y: 0, z: 50 }
    else targetPos.z = 50
  ...
  this.checkAnnBeforeBatchEdit = function () {
    ...
    if ((worldList.length < meta.frames.length && worldList.length <= 9) || !allLoaded) {
  ...
  this.interpolateInBackground = function () {
    if (!this.checkBoxTrackId()) return

    if (!this.checkAnnBeforeBatchEdit()) return
        ...
        switch ( ev.key) {
            ...
            */
            case 'z': // X
                ...
                break;
                case 'y':
                this.selected_box().forEach(e=>e.setSelected(true));
                break;
            case 'T':
            case 't':
                if (this.selected_box){                
                    this.boxOp.rotate_z(this.selected_box, 0.0, true);
                    this.on_box_changed(this.selected_box);
                }
            break;
	    case 'F2':
       ...
       case '7':
        ...
        break;
            ...
            case 'm' :
                if (this.selected_box){
                    //this.transform_bbox("z_rotate_left");
                    this.boxOp.rotate_z(this.selected_box, -1.5708, true);
                    this.on_box_changed(this.selected_box);
                }
                break;
            case 'n' :
                ...
                if (this.selected_box){
                    //this.transform_bbox("z_rotate_left");
                    this.boxOp.rotate_z(this.selected_box, -1.5708, false);
                    this.on_box_changed(this.selected_box);
                }
                break;
                case 'l' :
